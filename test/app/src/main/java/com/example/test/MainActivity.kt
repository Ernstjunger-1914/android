...
import android.app.Activity import android.content.Context import androidx.appcompat.app.AppCompatActivity import android.os.Bundle import android.os.Handler import android.os.Message import android.util.DisplayMetrics import android.view.KeyEvent import android.view.View import android.view.animation.* import android.view.inputmethod.InputMethodManager import android.widget.* import java.lang.StringBuilder import kotlin.random.Random
class MainActivity : AppCompatActivity() {
        ...
        for (i in 0..(tableLayout.childCount-1)) {
            ...
            for (i in 0..(tr.childCount-1)) {
                ...
                if (btn.id == -1) {
                    ...
                    btn.setOnClickListener { v: View -> val curIndex = editText.selectionStart
                        var passwordStr = editText.text.toString()
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        if(keyCode == KeyEvent.KEYCODE_BACK){ if(flipper.currentView.id == R.id.firstViewFlipper){
